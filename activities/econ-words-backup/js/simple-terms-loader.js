...
function loadTerms(callback) {
    ...
    const possiblePaths = [
        './data/econ-terms.csv', 
        '../data/econ-terms.csv',
        '/activities/econ-words/data/econ-terms.csv',
        'data/econ-terms.csv'
    ];
    console.log('Will try to fetch CSV file from these paths:', possiblePaths);
    ...
    function tryNextPath(pathIndex) {
        ...
        console.log(`Attempting to load CSV from path (${pathIndex + 1}/${possiblePaths.length}):`, currentPath);

        fetch(currentPath)
            .then(response => {
                console.log(`CSV response for ${currentPath}:`, response.status, response.statusText);
                
                if (!response.ok) {
                    // Try the next path
                    console.log(`Path ${currentPath} failed, trying next path...`);
                    tryNextPath(pathIndex + 1);
                    return Promise.reject(`CSV not found at ${currentPath}`);
                }
